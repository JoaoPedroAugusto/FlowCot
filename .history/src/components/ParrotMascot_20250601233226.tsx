...
interface ParrotMascotProps {
  position?: 'left' | 'right';
  message?: string;
  isMobile?: boolean; // Adicionando prop isMobile
}
...
const ParrotMascot: React.FC<ParrotMascotProps> = ({ position = 'right', isMobile = false }) => {
  ...
  // Ajuste de tamanho baseado no dispositivo
  ...
  const getResponse = async (question: string): Promise<string> => {
    ...
    setIsTyping(false);
    return 'Olá! Como posso ajudar?'; // Added default return to fix TypeScript error
